...
server.tool("get_verdict", "Get a trust verdict for an MCP package: allow, warn, or block. Based on scan findings (score and severity). Also reports monitoring status and publisher posture. Use this before installing or connecting to an MCP server.", {
    npm: z.string().describe("npm package name"),
}, async (args) => handleTool("get_verdict", args));
server.tool("check_exposure", "Check which monitored MCP servers depend on a given package. Use this during incident response to find blast radius. Example: 'which MCP servers use axios?'", {
    npm: z.string().describe("npm package name to check exposure for (e.g. 'axios')"),
}, async (args) => handleTool("check_exposure", args));
server.tool("check_abuse", "Check if a package or agent has been reported to the KYA abuse database. Returns whether abuse has been reported and any details.", {
    agent: z.string().describe("Package name or agent identifier to check"),
}, async (args) => handleTool("check_abuse", args));
server.tool("monitor_status", "Check if an MCP package is under continuous monitoring and get its scan history. Shows current score, risk level, and recent changes.", {
    npm: z.string().describe("npm package name"),
