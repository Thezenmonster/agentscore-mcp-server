...
function policyGateHint(options) {
    const lines = [
        "",
        "Policy Gate:",
...
async function fetchVerdictForDependency(dep) {
    try {
...
function formatRepoSummary(repoPath, repoUrl, checkedFiles, results) {
    const lines = [
        `Repo path: ${repoPath}`,
    ...
    lines.push("", `MCP dependencies found: ${results.length}`, `Allow: ${counts.allow} | Warn: ${counts.warn} | Block: ${counts.block} | Unknown: ${counts.unknown}`, "", "Dependency verdicts:");
    for (const item of results) {
        ...
        lines.push(`  ${item.name}@${version} - ${item.verdict.toUpperCase()} (${score}/100, ${risk}) [source: ${item.source}]${reasons}`);
    }
    lines.push("", "Next step:", "  Run generate_policy_gate_setup to preview the OIDC-based GitHub Actions workflow for this repo.", "  Run install_policy_gate to write .github/workflows/agentscore-policy-gate.yml directly.");
    return lines.join("\n");
}
function buildOidcWorkflowYaml(packages) {
    const lines = [
...
function formatPolicyGateSetup(repoPath, repoUrl, dependencies, checkedFiles) {
    ...
    if (explicitPackages.length === 0) {
        lines.push("", "Note:", "  No MCP packages were detected locally. The Action can still auto-detect root package.json dependencies,", "  but if this repo uses MCP packages via config files only, add them explicitly to the packages input.");
    }
    else {
        lines.push("", "Why this workflow uses OIDC:", "  GitHub Actions proves repo identity directly to AgentScore on each run.", "  No API key, no secret distribution, and no manual provisioning step are required.", "", "Why this workflow uses an explicit package list:", "  It keeps CI aligned with the MCP dependencies detected in this repo, even when they come from config files", "  and not just the root package.json.");
    }
    return lines.join("\n");
}
    ...
    try {
        switch (name) {
            case "scan_package": {
                const npm = args.npm;
                if (!npm?.trim())
                    return text("Package name is required.", true);
                const data = await fetchApi("/api/scan", { npm });
                if (data.status === "not_found")
                    return text(`Package "${npm}" not found on npm.`, true);
            ...
            case "check_abuse": {
                ...
                if (!agent?.trim())
                    return text("Agent or package name is required.", true);
