...
function formatRepoSummary(repoPath, repoUrl, checkedFiles, results) {
    ...
    lines.push("", "Next step:", "  Run generate_policy_gate_setup to get the exact GitHub Actions workflow for this repo.", `  Start a free pilot: ${buildIntentUrl("pilot", repoUrl)}`);
    return lines.join("\n");
}
function buildWorkflowYaml(packages) {
    const lines = [
...
function formatPolicyGateSetup(repoPath, repoUrl, dependencies, checkedFiles) {
    ...
    if (explicitPackages.length === 0) {
        ...
        lines.push("", "Why this workflow uses an explicit package list:", "  It keeps CI aligned with the MCP dependencies detected in this repo, even when they come from config files", "  and not just the root package.json.");
    }
    return lines.join("\n");
}
